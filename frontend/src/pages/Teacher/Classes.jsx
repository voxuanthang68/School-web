...
const TeacherClasses = () => {
  ...
  const [form, setForm] = useState({ name: '', subject_id: '', semester_id: '', room: '', schedule: '' });
  ...
  const handleCreateClass = async () => {
    try {
      ...
      setForm({ name: '', subject_id: '', semester_id: '', room: '', schedule: '' });
      fetchData();
    } catch (err) { alert(err.response?.data?.detail || 'Lỗi'); }
  };
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setShowCreateModal(true)}>+ Thêm lớp</button>
      </div>
                ...
                <tbody>
                  {(classDetail.approved_students_info || []).map(s => (
                    <tr key={s.id}>
                      ...
                      <td style={{ textAlign: 'right' }}><button className="btn btn-sm btn-danger" onClick={() => handleRemoveStudent(s.id)}>🗑 Xoá</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="modal-overlay" onClick={() => setShowCreateModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={handleCreateClass}>💾 Lưu</button>
            </div>
          </div>
        </div>
      )}
